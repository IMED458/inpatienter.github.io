...
<head>
  ...
  <script type="module">
    ...
    document.getElementById('edit-form').onsubmit = async function(e) {
      e.preventDefault();
      try {
        await updateDoc(doc(db, currentCollection.path, editingDocId), {
    ...
    window.clearAllData = async function() {
      ...
      if (!confirm("დარწმუნებული ხართ?")) return;
      const pass = prompt("დაადასტურეთ: „გასუფთავება2025“");
      if (pass !== "გასუფთავება2025") return showToast('გაუქმდა');
      try {
