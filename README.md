...
<head>
  ...
  <script type="module">
    ...
    // ფორმა: ახალი პაციენტის დამატება
    document.getElementById('patient-form')?.addEventListener('submit', async e => {
      ...
      try {
        ...
        showToast('შეცდომა დამატებისას', true);
      }
    });
    ...
    // სრული გასუფთავება (პაროლით)
    window.clearAllData = async function() {
      if (!confirm("ყველა პაციენტი წაიშლება სამუდამოდ!")) return;
      if (!confirm("დარწმუნებული ხართ?")) return;
      const pass = prompt("ჩაწერეთ: „გასუფთავება2025“");
      if (pass !== "გასუფთავება2025") {
        showToast("პაროლი არასწორია");
        return;
      }
      try {
        const snapshot = await getDocs(currentCollection);
        const deletes = snapshot.docs.map(d => deleteDoc(doc(db, currentCollection.path, d.id)));
        await Promise.all(deletes);
        showToast("დარბაზი მთლიანად გასუფთავდა");
      } catch (err) {
        showToast("გასუფთავება ვერ მოხერხდა", true);
      }
    };
    ...
    <div class="container">
      ...
      <div id="active-tab" class="tab-content active">
        <div class="card">
          ...
          <button type="button" class="clear-all-btn" onclick="clearAllData()">ყველას წაშლა (მთლიანი გასუფთავება)</button>
        </div>

        <div class="card">
          <h2>აქტიური პაციენტები</h2>
          <div class="search-filter"><input type="text" id="search" placeholder="ძებნა სახელით ან ისტორიით"></div>
